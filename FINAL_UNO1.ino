...
float SET_TEMP = 28.00;     // Threshold temperature setting (MAXIMUM)
float SET_HUM = 50.00;      // Threshold humidity setting (MAXIMUM)
...
void loop() {
  ...
  if (humidity > SET_HUM || nanoT > temperature) {             // If humidity is greater than SET_HUM threshold or outside (NANO) temp > inside (UNO) temp
